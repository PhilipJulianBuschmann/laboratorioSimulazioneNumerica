...
{ 
  ...
  int nconf = 1;
  for(int i = 0; i < 3000; i++) Move(); //Equilibration
...
//Output files saranno della forma:
//num blocco, val medio nel blocco, val medio progressivo, err progressivo
...
//Read input informations
  ...
  tail_pot = 8*3.14159265*rho*(1/(9*pow(rcut, 9))-1/(3*pow(rcut, 3)));
  cout << "Potential tail correction = " << tail_pot << endl;
  tail_prs = 32*3.14159265*rho*(1/(9*pow(rcut, 9))-1/(6*pow(rcut, 3)));
  cout << "Pression tail correction = " << tail_prs << endl << endl << endl;
...
//Read initial configuration
  ...
  else 
  {
    ...
    for (int i=0; i<npart; ++i)
    {
      vx[i] = vx[i] - sumv[0]; //Elimino la velocita di drift
      vy[i] = vy[i] - sumv[1];
      vz[i] = vz[i] - sumv[2];
  ...
  for (int i=0; i<npart; ++i)
  {
    ...
    x[i] = Pbc( x[i] * box ); //Trasformo da unita di misura del lato a unita di misura naturali
    y[i] = Pbc( y[i] * box ); //Pbc = periodic boundary condition
    z[i] = Pbc( z[i] * box );
  }
  ReadConf.close();
  ...
  for (int i=0; i<npart; ++i)
  {
    if(iNVET) //se invet = 1 fa montecarlo, non il nostro caso -> invet = 0 -> dinamica molecolare
    {
      xold[i] = x[i];
      yold[i] = y[i];
      zold[i] = z[i];
    ...
    {
      xold[i] = Pbc(x[i] - vx[i] * delta); //Posizioni vecchie -> algoritmo necessita di x[t-dt] e x[t]
...
// distance i-j in pbc
      ...
      w = n_props + 1;
      for(double r = 0; r < box/2; r += ddr) {
        double deltaV = (4*pi/3)*(pow(r + ddr, 3) - pow(r, 3));
        double constant = 1/(rho*double(npart)*deltaV);
        if(dr > r && dr < r + ddr){
           walker[w] += 2*constant;
        }
        w++;
  ...
  walker[iv] = 4.0 * v + tail_pot; // Potential energy //Moltiplico fuori dal ciclo il 4!!!
  walker[ik] = kin; // Kinetic energy
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature
  walker[ie] = 4.0 * v + kin;  // Total energy;
  walker[ip] = rho*temp + (1.0/(3.0*vol))*48.0*p + tail_prs*(double)npart/vol;*/
  //AGGIUNGERE QUI LA PRESSIONE
  return;
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pres, EpotInst, PresInst, grr, grr_block;
   const int wd=16;
   int w = n_props +1;
    ...
    grr.open("gr.dat", ios::app);
    grr_block.open("gr_block.dat", ios::app);
    ...
    /*Epot.open("output_epot.datGas",ios::app);
    Ekin.open("output_ekin.datGas",ios::app);
    Temp.open("output_temp.datGas",ios::app);
    Etot.open("output_etot.datGas",ios::app);
    Pres.open("output_pres.datGas",ios::app);
    EpotInst.open("op_EpotInst.dat", ios::app);
    PresInst.open("op_PresInst.dat");
    
    //moltiplicando per npart e dividendo per il volume
    stima_pot = blk_av[iv]/blk_norm/(double)npart; //Potential energy
    //EpotInst << stima_pot << endl;
    ...
    stima_pres = (blk_av[ip]+tail_prs)/blk_norm; //Pressure
    glob_av[ip] += stima_pres;
    glob_av2[ip] += stima_pres*stima_pres;
    err_pres=Error(glob_av[ip],glob_av2[ip],iblk);*/

    for(int i = 0; i < num_r; i++) {
      stima_pres = blk_av[i+w]/blk_norm;
      grr_block << stima_pres << endl;
      if(iblk == nblk) grr << stima_pres << endl; 
      glob_av[i+w] += stima_pres;
      glob_av2[i+w] += stima_pres*stima_pres;
      err_pres=Error(glob_av[i+w],glob_av2[i+w],iblk);
    }
/*
